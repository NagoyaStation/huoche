...
local function LoadActiveCharImages()
    ...
    if charDef.id == "warrior" then
        ...
        if heroSpineInst then
            ...
            -- 隐藏路径约束相关slot（UrhoX不支持path约束，显示会异常）
            heroSpineInst:SetAttachment("path_tui_2", nil)
            heroSpineInst:SetAttachment("path_day_1", nil)
            heroSpineInst:SetAttachment("tui_s", nil)
            heroSpineInst:SetAttachment("tui_1", nil)
            heroSpineInst:SetAttachment("tui_2", nil)
            heroSpineInst:SetAttachment("tui_3", nil)
